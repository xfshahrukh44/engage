...
">
                            ...
                            <div class="tab">What's your date of birth?
                                <p><input placeholder="dd" type="date" oninput="this.className = ''" name="dob">
                                </p>
...
@section('js')
    ...
    <script>
        ...
        function validateForm() {
            var x = document.getElementsByClassName("tab");
            var tab = x[currentTab];
            var valid = true;

            // check all input/select fields
            ...
            inputs.forEach(function(input) {
                ...
                if (type === "radio") {
                    let name = input.name;
                    let radios = tab.querySelectorAll("input[name='" + name + "']");
                    oneChecked = Array.from(radios).some(r => r.checked);
                    if (!oneChecked) valid = false;
                } else if (type === "checkbox") {
                    // For checkbox group (optional — you can require at least one)
                    let checkboxes = tab.querySelectorAll("input[type='checkbox']");
                    if (checkboxes.length > 0) {
                        oneChecked = Array.from(checkboxes).some(c => c.checked);
                        if (!oneChecked) valid = false;
                    }
                } else {
                    // For text, email, number, select
                    if (input.value.trim() === "") {
            ...
            // optional red border effect for invalids
            if (!valid) {
